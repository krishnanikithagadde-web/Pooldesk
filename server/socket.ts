...
export function initSocketServer(httpServer: http.Server) {
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('otpVerified', (data: { rideId: string }) => {
      const { rideId } = data;
      if (!rideId) return;

      // Broadcast OTP verified to all users in this ride
      io?.to(`ride_${rideId}`).emit('otpVerified', data);
      console.log(`OTP verified event broadcasted for ride ${rideId}`);
    });

    socket.on('rideCompleted', (data: { rideId: string; payment?: any }) => {
      const { rideId, payment } = data;
      if (!rideId) return;

      // Broadcast ride completion to all users in this ride
      io?.to(`ride_${rideId}`).emit('rideCompleted', {
        rideId,
        payment,
      });
      console.log(`Ride completed event broadcasted for ride ${rideId}`);
    });
