...
import { MapPin, Users, ChevronDown, ChevronUp, Star } from "lucide-react";
...
export default function RideResults({
  ...
  const handleBookRide = async (seatsToBook: number, passengerDetails: PassengerGender[]) => {
    ...
    try {
      ...
      // Fetch booking details from server to get canonical passengerId, then join socket rooms, seed cache and redirect
      try {
        const bookingDetailsRes = await fetch(`/api/bookings/${data.booking.id}`);
        let canonicalPassengerId = passengerId;
        if (bookingDetailsRes.ok) {
          const bookingDetails = await bookingDetailsRes.json();
          canonicalPassengerId = bookingDetails.passengerId || bookingDetails.passenger_id || canonicalPassengerId;
        }

        // Join socket and seed cache
        import('@/lib/socket').then(({ getSocket }) => {
          try {
            const socket = getSocket();
            socket.emit('joinRide', { rideId: selectedRide.ride_id, userType: 'passenger' });
            if (canonicalPassengerId) {
              socket.emit('joinPassenger', { passengerId: canonicalPassengerId });
            }

            // Persist booking cache with canonical passenger id
            const bookingKey = `booking_${selectedRide.ride_id}`;
            const bookingCache = {
              bookingId: data.booking.id,
              rideId: selectedRide.ride_id,
              ...
              driverEmail: selectedRide.driverEmail || '',
              driverInfo: {
                name: selectedRide.driverName || '',
                email: selectedRide.driverEmail || '',
                carBrand: selectedRide.carBrand || '',
              ...
              carModel: selectedRide.carModel || '',
              carLicensePlate: '',
              otp: '', // OTP will be filled when driver accepts (socket event)
              timestamp: Date.now(),
            };

            localStorage.setItem(bookingKey, JSON.stringify(bookingCache));
            localStorage.setItem('userType', 'passenger');

            socket.on('bookingAccepted', (payload: any) => {
              if (payload && payload.rideId === selectedRide.ride_id) {
                localStorage.setItem('userType', 'passenger');
                // merge OTP into booking cache if present
                if (payload.otp) {
                  const cached = JSON.parse(localStorage.getItem(bookingKey) || '{}');
                  cached.otp = payload.otp;
                  cached.driverInfo = payload.driverInfo || cached.driverInfo;
                  localStorage.setItem(bookingKey, JSON.stringify(cached));
                }
                window.location.href = `/active-ride/${selectedRide.ride_id}`;
              }
            });

            // finally redirect so passenger sees ActiveRide
            window.location.href = `/active-ride/${selectedRide.ride_id}`;
          } catch (e) {
            console.warn('Socket join failed, falling back to redirect + cache seed', e);
            const bookingKey = `booking_${selectedRide.ride_id}`;
            const bookingCache = {
              bookingId: data.booking.id,
              rideId: selectedRide.ride_id,
          ...
          console.warn('Failed to import socket module:', e);
          const bookingKey = `booking_${selectedRide.ride_id}`;
          const bookingCache = {
            bookingId: data.booking.id,
            rideId: selectedRide.ride_id,
        ...
        // Fallback: seed cache with provided passengerId and redirect
        const bookingKey = `booking_${selectedRide.ride_id}`;
        const bookingCache = {
          bookingId: data.booking.id,
          rideId: selectedRide.ride_id,
        ...
        >
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex-1">
              {ride.driverId ? (
                <Link to={`/driver/${ride.driverId}/profile`}>
                  <h4 className="text-lg font-bold text-blue-600 hover:underline cursor-pointer">
                    {ride.driverName || "Anonymous Driver"}
                  </h4>
                </Link>
              ) : (
                    ...
                    <div className="flex items-center">
                      {[...Array(5)].map((_, i) => (
                        <Star
                          key={i}
                          className={`h-3 w-3 ${
                            i < Math.floor(ride.driverAverageRating!) ? "fill-yellow-400 text-yellow-400" : "text-gray-300"
                          }`}
                        />
                      ))}
                    </div>
                    <p className="text-xs text-gray-600">({ride.driverAverageRating.toFixed(1)})</p>
                  </div>
                ) : (
